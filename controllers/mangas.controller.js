...
//FN que actualiza solamente la cantidad del manga
const mangaQTYPatch = async (req = request, res = response) => {
	try {
		const { id } = req.params;
		const { cantidad } = req.body;
		const manga = await Manga.findByIdAndUpdate(id, { cantidad });
		res.status(200).json({
			manga,
		});
	} catch (error) {
...
//FN que actualiza solamente el estado del manga
const mangaStatePatch = async (req = request, res = response) => {
	try {
		const { id } = req.params;
		const { estado } = req.body;
		const manga = await Manga.findByIdAndUpdate(id, { estado });
		res.status(200).json({
			manga,
		});
	} catch (error) {
